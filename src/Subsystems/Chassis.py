...
chassis : SmartDrive = SmartDrive(LeftMotor,
                                  RightMotor,
                                  imu,
...
def turnToHeading(targetHeading):
    ...
    while InertialSens.getHeading() != InertialSens.setheading(targetHeading):
        ...
        error = (error + 180) % 360 - 180  # gets the error between the range (-180, 180)
        ...
        motorPower = max(min(motorPower, 100), -100) # Motor power is limited to between -100, 100
